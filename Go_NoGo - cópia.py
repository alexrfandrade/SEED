...
def run_gonogo_task(win=None):
    ...
    if not os.path.exists(IMAGE_FOLDER):
        ...
        print("Please create a folder named 'go_nogo_stim' with your stimulus images.")
    ...
    def show_instructions_with_images():
       ...
       # Título
        ...
        win.flip()
        event.waitKeys(keyList=['space', 'escape']) #####RETIRAR - key buffer issue: 
    ...
    for i, trial in enumerate(practice_trials):
        ...
        if keys:
            key, rt = keys[0]
            if key == 'escape': ##### REMOVE - we dont need escape ####
    ...
    # Randomizar trials (exceto os primeiros 21# que são sempre Go)
    remaining_trials = all_trials[21:]
    random.shuffle(remaining_trials)
    all_trials = all_trials[:21] + remaining_trials

    # Loop principal
    for trial_num, trial in enumerate(all_trials, 1):
        print(f"  Trial {trial_num}/{N_TRIALS}: {trial['type']} - {trial['description']}")
        
        # Fixação com jitter ##### CHANGE (check ref paper) #######
        fixation.draw()
        win.flip()
        ...
        # Coletar resposta  #####CHANGE (key buffer) ######
        clock = core.Clock()
        keys = event.waitKeys(maxWait=1.0, keyList=['space', 'escape'], 
                             timeStamped=clock, clearEvents=True)
        ...
        if keys:
            key, rt = keys[0]
            if key == 'escape': ######REMOVE we dont want escape option
                break
            response = key
