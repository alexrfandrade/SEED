...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\IBEB\\Documents\\GitHub\\SEED\\test_trigger_SEED_EEG_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "EEG_open" ---
    # Run 'Begin Experiment' code from code_EEG_open
    import time
    ...
    text_Explain_PAL = visual.TextStim(win=win, name='text_Explain_PAL',
        text='Na primeira tarefa, vamos mostrar um par de imagens : um local e um objeto. Analisa se há uma associação entre o par. \n\nAo fim de 2,5 SEGUNDOS, vai aparecer uma barra de avaliação abaixo, para selecionares o nível de associação entre o par onde:\n1 = Pouco relacionados\n2 = Intermédio\n3 = Fortemente relacionados\n\nPrime ESPAÇO para avançar\n',
        font='Arial',
    ...
    text_explain_rate = visual.TextStim(win=win, name='text_explain_rate',
        text="Prime a tecla indicada como '1', '2', ou '3' no teclado numérico (ao lado das flechas) para selecionar o nível de associação entre o par.\n\nTens 2,5SEGUNDOS para responder. Se te enganares ou não responderes a tempo em alguma(s), não faz mal. Continua!\n\nPrime ESPAÇO para um exemplo",
        font='Arial',
    ...
    slider_example = visual.Slider(win=win, name='slider_example',
        ...
        labels=['1 - Pouco Relacionados','2 - Intermédio','3 - Fortemente Relacionados'], ticks=(1, 2, 3), granularity=1.0,
    ...
    # --- Initialize components for Routine "EEG_close" ---
    ...
    # --- Prepare to start Routine "EEG_open" ---
    # create an object to store info about Routine EEG_open
    EEG_open = data.Routine(
        name='EEG_open',
        components=[],
    )
    EEG_open.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # store start times for EEG_open
    EEG_open.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    EEG_open.tStart = globalClock.getTime(format='float')
    EEG_open.status = STARTED
    thisExp.addData('EEG_open.started', EEG_open.tStart)
    EEG_open.maxDuration = None
    # keep track of which components have finished
    EEG_openComponents = EEG_open.components
    for thisComponent in EEG_open.components:
    ...
    # --- Run Routine "EEG_open" ---
    EEG_open.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        ...
        if not continueRoutine:  # a component has requested a forced-end of Routine
            EEG_open.forceEnded = routineForceEnded = True
            break
    ...
    # --- Ending Routine "EEG_open" ---
    for thisComponent in EEG_open.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for EEG_open
    EEG_open.tStop = globalClock.getTime(format='float')
    EEG_open.tStopRefresh = tThisFlipGlobal
    thisExp.addData('EEG_open.stopped', EEG_open.tStop)
    thisExp.nextEntry()
    # the Routine "EEG_open" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisExemplo_PAL in exemplo_PAL:
        ...
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            if cross_25.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if image_blank.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_blank.tStartRefresh + 1-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        while continueRoutine and routineTimer.getTime() < 2.0:
            ...
            # if image_ex_scene is stopping this frame...
            if image_ex_scene.status == STARTED:
                # is it time to stop? (based on local clock)
                if tThisFlip > 2-frameTolerance:
                    # keep track of stop time/frame for later
                    image_ex_scene.tStop = t  # not accounting for scr refresh
                    image_ex_scene.tStopRefresh = tThisFlipGlobal  # on global time
                    image_ex_scene.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'image_ex_scene.stopped')
                    # update status
                    image_ex_scene.status = FINISHED
                    image_ex_scene.setAutoDraw(False)
            ...
            # if image_ex_object is stopping this frame...
            if image_ex_object.status == STARTED:
                # is it time to stop? (based on local clock)
                if tThisFlip > 2-frameTolerance:
                    # keep track of stop time/frame for later
                    image_ex_object.tStop = t  # not accounting for scr refresh
                    image_ex_object.tStopRefresh = tThisFlipGlobal  # on global time
                    image_ex_object.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'image_ex_object.stopped')
                    # update status
                    image_ex_object.status = FINISHED
                    image_ex_object.setAutoDraw(False)
            
            # *slider_example* updates
            
            # if slider_example is starting this frame...
            if slider_example.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if slider_example is stopping this frame...
            if slider_example.status == STARTED:
                # is it time to stop? (based on local clock)
                if tThisFlip > 2-frameTolerance:
                    # keep track of stop time/frame for later
                    slider_example.tStop = t  # not accounting for scr refresh
                    slider_example.tStopRefresh = tThisFlipGlobal  # on global time
                    slider_example.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'slider_example.stopped')
                    # update status
                    slider_example.status = FINISHED
                    slider_example.setAutoDraw(False)
            
            # *text_example* updates
            
            # if text_example is starting this frame...
            if text_example.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if text_example is stopping this frame...
            if text_example.status == STARTED:
                # is it time to stop? (based on local clock)
                if tThisFlip > 2-frameTolerance:
                    # keep track of stop time/frame for later
                    text_example.tStop = t  # not accounting for scr refresh
                    text_example.tStopRefresh = tThisFlipGlobal  # on global time
                    text_example.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'text_example.stopped')
                    # update status
                    text_example.status = FINISHED
                    text_example.setAutoDraw(False)
            
            # *key_ex_rate* updates
            waitOnFlip = False
            
            # if key_ex_rate is starting this frame...
            if key_ex_rate.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if key_ex_rate is stopping this frame...
            if key_ex_rate.status == STARTED:
                # is it time to stop? (based on local clock)
                if tThisFlip > 2-frameTolerance:
                    # keep track of stop time/frame for later
                    key_ex_rate.tStop = t  # not accounting for scr refresh
                    key_ex_rate.tStopRefresh = tThisFlipGlobal  # on global time
                    key_ex_rate.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'key_ex_rate.stopped')
                    # update status
                    key_ex_rate.status = FINISHED
                    key_ex_rate.status = FINISHED
            if key_ex_rate.status == STARTED and not waitOnFlip:
                ...
                if len(_key_ex_rate_allKeys):
                    key_ex_rate.keys = _key_ex_rate_allKeys[0].name  # just the first key pressed
                    key_ex_rate.rt = _key_ex_rate_allKeys[0].rt
                    key_ex_rate.duration = _key_ex_rate_allKeys[0].duration
                    # a response ends the routine
                    continueRoutine = False
        ...
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if Example_PAL.maxDurationReached:
            routineTimer.addTime(-Example_PAL.maxDuration)
        elif Example_PAL.forceEnded:
            routineTimer.reset()
        else:
    ...
    for thisTrial_encode in trial_encode:
        ...
        key_rate.rt = []
        _key_rate_allKeys = []
        # Run 'Begin Routine' code from EEG_obj_encode
        if hasattr(image_object_encode, 'trigger_sent'):
            del image_object_encode.trigger_sent
        
        # Run 'Begin Routine' code from EEG_rating
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            # if image_scene_encode is stopping this frame...
            if image_scene_encode.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_scene_encode.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
                    image_scene_encode.tStop = t  # not accounting for scr refresh
                    image_scene_encode.tStopRefresh = tThisFlipGlobal  # on global time
                    image_scene_encode.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'image_scene_encode.stopped')
                    # update status
                    image_scene_encode.status = FINISHED
                    image_scene_encode.setAutoDraw(False)
            ...
            # if image_object_encode is stopping this frame...
            if image_object_encode.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_object_encode.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
                    image_object_encode.tStop = t  # not accounting for scr refresh
                    image_object_encode.tStopRefresh = tThisFlipGlobal  # on global time
                    image_object_encode.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'image_object_encode.stopped')
                    # update status
                    image_object_encode.status = FINISHED
                    image_object_encode.setAutoDraw(False)
            
            # *text_rating* updates
            
            # if text_rating is starting this frame...
            if text_rating.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if text_rating is stopping this frame...
            if text_rating.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > text_rating.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    text_rating.tStop = t  # not accounting for scr refresh
                    text_rating.tStopRefresh = tThisFlipGlobal  # on global time
                    text_rating.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'text_rating.stopped')
                    # update status
                    text_rating.status = FINISHED
                    text_rating.setAutoDraw(False)
            
            # *slider_cong* updates
            
            # if slider_cong is starting this frame...
            if slider_cong.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if slider_cong is stopping this frame...
            if slider_cong.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > slider_cong.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
                    slider_cong.tStop = t  # not accounting for scr refresh
                    slider_cong.tStopRefresh = tThisFlipGlobal  # on global time
                    slider_cong.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'slider_cong.stopped')
                    # update status
                    slider_cong.status = FINISHED
                    slider_cong.setAutoDraw(False)
            ...
            if key_rate.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_rate.tStartRefresh + 2.5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_rate.status == STARTED and not waitOnFlip:
                ...
                if len(_key_rate_allKeys):
                    key_rate.keys = _key_rate_allKeys[0].name  # just the first key pressed
                    key_rate.rt = _key_rate_allKeys[0].rt
                    key_rate.duration = _key_rate_allKeys[0].duration
                    # a response ends the routine
                    continueRoutine = False
            # Run 'Each Frame' code from EEG_obj_encode
    ...
    while continueRoutine:
        ...
        if key_end_cog.status == NOT_STARTED and tThisFlip >= 15-frameTolerance:
            # keep track of start time/frame for later
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            if key_ex_old.status == STARTED and not waitOnFlip:
                ...
                if len(_key_ex_old_allKeys):
                    key_ex_old.keys = _key_ex_old_allKeys[0].name  # just the first key pressed
                    key_ex_old.rt = _key_ex_old_allKeys[0].rt
                    key_ex_old.duration = _key_ex_old_allKeys[0].duration
    ...
    for thisTrial_item in trial_item:
        ...
        key_item.rt = []
        _key_item_allKeys = []
        # Run 'Begin Routine' code from code_EEG_old
        if hasattr(image_old_new, 'trigger_sent'):
            del image_old_new.trigger_sent
        
        # Run 'Begin Routine' code from EEG_key_old
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            if key_item.status == STARTED and not waitOnFlip:
                ...
                if len(_key_item_allKeys):
                    key_item.keys = _key_item_allKeys[0].name  # just the first key pressed
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            if key_ex_2AFC.status == STARTED and not waitOnFlip:
                ...
                if len(_key_ex_2AFC_allKeys):
                    key_ex_2AFC.keys = _key_ex_2AFC_allKeys[0].name  # just the first key pressed
                    key_ex_2AFC.rt = _key_ex_2AFC_allKeys[0].rt
                    key_ex_2AFC.duration = _key_ex_2AFC_allKeys[0].duration
    ...
    for thisTrial_asso in trial_asso:
        ...
        # Run 'Begin Routine' code from EEG_image_AFC
        if hasattr(image_target, 'trigger_sent'):
            del image_target.trigger_sent
        
        # Run 'Begin Routine' code from EEG_Key_2AFC
        if hasattr(key_2AFC, 'asso_trigger_sent'):
            del key_2AFC.asso_trigger_sent
    ...
    while continueRoutine and routineTimer.getTime() < 8.0:
        ...
        if text_end.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > text_end.tStartRefresh + 8.0-frameTolerance:
                # keep track of stop time/frame for later
    ...
    # --- Prepare to start Routine "EEG_close" ---
    # create an object to store info about Routine EEG_close
    EEG_close = data.Routine(
        name='EEG_close',
        components=[],
    )
    EEG_close.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from EEG_close
    rcs.sendAnnotation('End', 'Init')
    # store start times for EEG_close
    EEG_close.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    EEG_close.tStart = globalClock.getTime(format='float')
    EEG_close.status = STARTED
    thisExp.addData('EEG_close.started', EEG_close.tStart)
    EEG_close.maxDuration = None
    # keep track of which components have finished
    EEG_closeComponents = EEG_close.components
    for thisComponent in EEG_close.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "EEG_close" ---
    EEG_close.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        # Run 'Each Frame' code from EEG_close
        ret, frame= map.read()
        if ret:
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            EEG_close.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in EEG_close.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "EEG_close" ---
    for thisComponent in EEG_close.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for EEG_close
    EEG_close.tStop = globalClock.getTime(format='float')
    EEG_close.tStopRefresh = tThisFlipGlobal
    thisExp.addData('EEG_close.stopped', EEG_close.tStop)
    thisExp.nextEntry()
    # the Routine "EEG_close" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    # Run 'End Experiment' code from code_EEG_open
