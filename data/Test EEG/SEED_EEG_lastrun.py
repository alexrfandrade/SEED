...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    for thisExemplo_PAL in exemplo_PAL:
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            if image_ex_scene.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_ex_scene.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if image_ex_object.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > image_ex_object.tStartRefresh + 5-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    while continueRoutine:
        ...
        if key_start_PAL.status == STARTED and not waitOnFlip:
            ...
            if len(_key_start_PAL_allKeys):
                key_start_PAL.keys = _key_start_PAL_allKeys[0].name  # just the first key pressed
                key_start_PAL.rt = _key_start_PAL_allKeys[0].rt
                key_start_PAL.duration = _key_start_PAL_allKeys[0].duration
                # a response ends the routine
                continueRoutine = False
    ...
    for thisTrial_encode in trial_encode:
        ...
        key_rate.rt = []
        _key_rate_allKeys = []
        # Run 'Begin Routine' code from EEG_object_encode
        ...
        while continueRoutine and routineTimer.getTime() < 5.0:
            ...
            # Run 'Each Frame' code from EEG_object_encode
    ...
    while continueRoutine:
        ...
        if key_Start_Cog.status == NOT_STARTED and t >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            ...
            # keyboard checking is just starting
            key_Start_Cog.clock.reset()  # now t=0
        if key_Start_Cog.status == STARTED:
    ...
    while continueRoutine:
        ...
        if key_end_cog.status == NOT_STARTED and tThisFlip >= 10-frameTolerance:
            # keep track of start time/frame for later
    ...
    for thisTrial_item in trial_item:
        ...
        key_item.rt = []
        _key_item_allKeys = []
        # Run 'Begin Routine' code from EEG_image_old
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            # Run 'Each Frame' code from EEG_image_old
    ...
    for thisTrial_asso in trial_asso:
        ...
        # Run 'Begin Routine' code from EEG_key_AFC
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            # Run 'Each Frame' code from EEG_key_AFC
    ...
    continueRoutine = True
    # update component parameters for each repeat
    # Run 'Begin Routine' code from code_EEG_close
    ...
    while continueRoutine:
        ...
        frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
        # update/draw components on each frame
        # Run 'Each Frame' code from code_EEG_close
        ret, frame= map.read()
        if ret:
